"""
Training script for standalone unconditional motion flow matching model.
...
from src.model.motion_flow_uncond import MotionFlowMatching
...
from src.evaluate.evaluator_flow_uncond import evaluate_flow
...
def build_model(args, device):
    model = MotionFlowMatching(
        ...
        t_scale=getattr(args, "t_scale", 1000.0),
...
@torch.no_grad()
def sample_and_visualize(model, args, device, mean, std, epoch, num_samples=4):
    """Generate samples via ODE and save visualizations."""
    model.eval()
    T = args.T - 1
...
def main():
    ...
    for epoch in tqdm(range(start_epoch, args.epochs + 1), desc="Epochs"):
        ...
        if eval_every > 0 and (epoch % eval_every == 0 or epoch == start_epoch):
            ...
            # MPJPE evaluation (interpolate -> ODE denoise -> compare with GT)
            _eval_noise_t = getattr(args, "eval_noise_t", 0.5)
            _eval_ode_steps = getattr(args, "eval_ode_steps", 30)
            _eval_solver = getattr(args, "eval_solver", "heun")
